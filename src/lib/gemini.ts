...
export async function generateSinglePlatformContent(content: string, platform: 'x' | 'instagram' | 'note', inputType: 'url' | 'text') {
  try {
...
【X用数字重視画像の例】
- 「上部に『売上成長率』のタイトル、中央に巨大な青い『30%』の文字、上昇する矢印グラフ、白背景、右下角は空白」
- 「『副業収入』タイトル、中央に金色の『月50万円』の大きな数字、コインアイコン、成功を表現する上向きグラフ、白背景、右下角は空白」
- 「『成功への道のり』タイトル、大きな『3ステップ』表示、1→2→3の縦長フローチャート、カラフルなアイコン、白背景、右下角は空白」
...
export async function generateContentWithGemini(content: string, inputType: 'url' | 'text') {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' })
...
export async function extractContentFromUrl(url: string): Promise<string> {
  try {
    ...
    // 長すぎる場合は適切な長さに制限
    return textContent.length > 3000 ? textContent.substring(0, 3000) + '...' : textContent
  } catch (error) {
